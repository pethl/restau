  ...
  def show_team
   if Accruel_rate.all.count == 0
    ...
    @table_dates=[]
      ...
      @pure_foh_staffhours =Staffhour.joins(:staff).where(staffs: {area: "Front of House", payment_terms: "Hourly Rate"}).where('week_end_date BETWEEN ? AND ?', start_date, end_date )
   ...
   def download_tronc_allocation_report_pdf
     ...
     respond_to do |format|
      format.pdf do
        ...
        pdf.text "Splits - FoH: "+ "£#{(sprintf "%.2f",@foh_split)}"+" Kitchen: "+ "£#{(sprintf "%.2f",@kit_split)}"+"\n", size: 8 
        pdf.text "Splits - FoH: "+get_foh_percentage.to_s+"%  Kitchen: "+ get_kitchen_percentage.to_s+"%\n", size: 8 
        pdf.text "\n", size: 6  
        #DOCUMENT SETUP_END  
