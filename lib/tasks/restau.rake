namespace :restau do
  ...
  task purge_held_bookings: :environment do
   ...
   task :send_booking_reminder => :environment do
     ...
     reminders_count = Booking.where("booking_date_time BETWEEN ? AND ?", Date.tomorrow.beginning_of_day, Date.tomorrow.end_of_day).where("status = ?", "Confirmed").count
  #    Booking.where("booking_date_time BETWEEN ? AND ?", Date.tomorrow.beginning_of_day, Date.tomorrow.end_of_day).where("status = ?", "Confirmed").with_index do |booking, index|
            
   #         if booking.all
   #           puts "#{index}/#{reminders_count} - #{booking.email}".green
   #         else
   #           puts "#{index}/#{reminders_count} - #{booking.email} - #{recalculate_badges.errors}".red
   #         end
   #       end

    #  puts "SEND_BOOKING_REMINDER_____RUN_TASK:Purging held bookings created yesterday..."
    #  Booking.where("created_at < ? AND status = ?", Date.yesterday.end_of_day, "Held").destroy_all
      puts "SEND_BOOKING_REMINDER_____AFTER_CHECK:Reminders send - count #{Booking.where("booking_date_time BETWEEN ? AND ?", Date.tomorrow.beginning_of_day, Date.tomorrow.end_of_day).where("status = ?", "Confirmed").count}" 
